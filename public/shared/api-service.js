...
function transformReservationPayload(payload) {
  ...
  if (payload.farm_option) {
    ...
    } else if (payload.farm_option === 'in-house' || payload.farm_option === 'farm-in' || payload.farm_option === 'farm_in' || payload.farm_option === 'in_house') {
      // Setting to in-house or farm-in: clear farmout status so it leaves farmout list
      result.farmout_status = null;
      result.farmout_mode = null;
      console.log(`📦 Farm option changed to ${payload.farm_option} - clearing farmout fields`);
    }
  }
  if (payload.farmOption) {
    ...
    if (payload.farmOption === 'farm-out') {
      ...
      result.farmout_mode = result.farmout_mode || 'manual';
      result.farmout_status = result.farmout_status || 'unassigned';
    } else if (payload.farmOption === 'in-house' || payload.farmOption === 'farm-in' || payload.farmOption === 'farm_in' || payload.farmOption === 'in_house') {
      // Setting to in-house or farm-in: clear farmout status so it leaves farmout list
      result.farmout_status = null;
      result.farmout_mode = null;
      console.log(`📦 Farm option changed to ${payload.farmOption} - clearing farmout fields`);
    }
  }
  ...
  if (payload.details) {
    ...
    const FARMOUT_STATUS_MAP = {
      ...
      // Additional farmout workflow statuses
      'on_the_way': 'on_the_way',
      'on-the-way': 'on_the_way',
      'ontheway': 'on_the_way',
      'customer_in_car': 'customer_in_car',
      'customer-in-car': 'customer_in_car',
      'customerincar': 'customer_in_car',
      'pob': 'customer_in_car',
      'completed': 'completed'
    };
    
    // Statuses that are valid for farmout reservations (should NOT clear farmout fields)
    const VALID_FARMOUT_WORKFLOW_STATUSES = new Set([
      'unassigned', 'assigned', 'offered', 'on_the_way', 'customer_in_car', 'completed',
      'farmout_unassigned', 'farmout_assigned', 'farmout_offered', 
      'farm_out_unassigned', 'farm_out_assigned', 'farm_out_offered',
      'on-the-way', 'ontheway', 'customer-in-car', 'customerincar', 'pob'
    ]);
    
    if (d.status) {
      const statusLower = d.status.toLowerCase().replace(/ /g, '_').replace(/-/g, '_');
      
      // Check if this is a farmout status (in the map or contains 'farm')
      ...
      } else if (VALID_FARMOUT_WORKFLOW_STATUSES.has(statusLower)) {
        // It's a valid farmout workflow status (on_the_way, customer_in_car, completed, etc.)
        // Update farmout_status but DON'T clear farmout_mode
        result.farmout_status = statusLower;
        result.status = statusLower === 'completed' ? 'completed' : 'in_progress';
        console.log(`📦 Farmout workflow status: ${d.status} -> farmout_status:${statusLower}, preserving farmout_mode`);
      } else if (VALID_DB_STATUSES.includes(statusLower)) {
        // It's a valid regular status - CLEAR farmout fields since it's no longer a farmout status
        result.status = statusLower;
        // Clear farmout fields when status is changed to a non-farmout status
        result.farmout_status = null;
        result.farmout_mode = null;
        console.log(`📦 Status changed to non-farmout: ${d.status} -> clearing farmout_status and farmout_mode`);
      } else if (UI_STATUS_MAP[statusLower]) {
        // Map UI status to valid database status - check if it should preserve farmout
        result.status = UI_STATUS_MAP[statusLower];
        // Only clear farmout fields if not a farmout workflow status
        if (!VALID_FARMOUT_WORKFLOW_STATUSES.has(statusLower)) {
          result.farmout_status = null;
          result.farmout_mode = null;
          console.log(`📦 Mapped UI status: ${d.status} -> ${result.status}, clearing farmout fields`);
        } else {
          console.log(`📦 Mapped UI status: ${d.status} -> ${result.status}, preserving farmout fields`);
        }
      } else {
    ...
    if (d.eFarmOut || d.farmoutMode) {
      ...
      // Check if mode is 'in-house' or empty (clearing farmout)
      if (fMode === 'in-house' || fMode === 'in_house' || fMode === '' || fMode === 'none') {
        result.farmout_mode = null;
        result.farmout_status = null;
        console.log(`📦 eFarmOut cleared to ${fMode} - clearing farmout fields`);
      } else {
        result.farmout_mode = fMode;
      ...
      if (d.farmOption === 'in-house' || d.farmOption === 'farm-in' || d.farmOption === 'farm_in' || d.farmOption === 'in_house') {
        // Clear farmout fields when switching to in-house or farm-in
        result.farmout_mode = null;
        result.farmout_status = null;
        console.log(`📦 farmOption changed to ${d.farmOption} - clearing farmout fields`);
