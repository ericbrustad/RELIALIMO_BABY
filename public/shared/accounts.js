...
class Accounts {
  ...
  /**
   * Save an email to the account_emails table
   * @param {string} accountId - UUID of the account
   * @param {string} email - Email address to save
   * @param {string} emailType - Type of email: 'primary', 'alternate', 'confirmation', etc.
   */
  async saveAccountEmail(accountId, email, emailType = 'primary') {
    if (!accountId || !email) {
      console.warn('⚠️ saveAccountEmail: Missing accountId or email');
      return null;
    }
    ...
    console.log('📧 Saving account email:', emailData);
    
    if (!window.supabase) {
      console.warn('⚠️ Supabase not available, skipping account_emails save');
      return null;
    }
    ...
    try {
      ...
      if (error) {
        ...
        if (error.code === '23505') {
          console.log('📧 Email already exists in account_emails (skipping)');
          return null;
        }
        throw error;
      }
      
      console.log('✅ Email saved to account_emails:', data);
      return data;
    } catch (e) {
      console.error('Error saving account email:', e);
      return null;
    }
  }
  ...
  async saveAccount() {
    ...
    try {
      ...
      // ========================================
      // Save primary email to account_emails for email dropdown
      // ========================================
      if (saved.id && accountData.email) {
        try {
          console.log('📧 Saving primary email to account_emails...');
          await this.saveAccountEmail(saved.id, accountData.email, 'primary');
          console.log('✅ Primary email saved to account_emails');
        } catch (emailErr) {
          console.warn('⚠️ Failed to save email to account_emails:', emailErr);
          // Don't block account creation if email save fails
        }
      }
