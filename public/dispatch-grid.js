class DispatchGrid {
  ...
  updateLiveVehicleMarkers(liveData) {
    ...
    this.liveDrivers = liveData.map(loc => {
      ...
      const driverName = driver ? `${driver.first_name || ''} ${driver.last_name || ''}`.trim() : `Driver ${loc.driver_id?.substring(0, 8) || 'Unknown'}`;
      return {
        id: loc.driver_id,
        lat: loc.latitude,
        lng: loc.longitude,
        ...
        phone: driver?.home_phone || '',
        email: driver?.email || '',
        driverType: driver?.driver_type || '',
        company: driver?.affiliated_company || '',
    ...
    if (this.gpsMap) {
      ...
      liveData.forEach(loc => {
        const driver = loc.drivers;
        const driverName = driver ? `${driver.first_name || ''} ${driver.last_name || ''}`.trim() : `Driver ${loc.driver_id?.substring(0, 8) || 'Unknown'}`;
        const phone = driver?.home_phone ? `<br>📞 <a href="tel:${driver.home_phone}">${driver.home_phone}</a>` : '';
        const statusIcon = loc.is_moving ? '🚗' : '📍';
        const speedMph = loc.speed ? (loc.speed * 2.237).toFixed(1) : '0';
        const battery = loc.battery_level != null ? `<br>🔋 ${loc.battery_level}%` : '';
        
        const markerIcon = L.divIcon({
          className: `vehicle-marker ${driver?.driver_status || 'available'} live-marker`,
          html: statusIcon,
          iconSize: [40, 40]
        });
        ...
        const marker = L.marker([loc.latitude, loc.longitude], { icon: markerIcon })
          .addTo(this.gpsMap)
          .bindPopup(`
            <strong>🟢 LIVE - ${driverName}</strong>${phone}<br>
            ${loc.is_moving ? '🚗 Moving' : '📍 Stationary'} - ${speedMph} mph${battery}<br>
