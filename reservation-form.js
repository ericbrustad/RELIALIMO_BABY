...
class ReservationForm {
  ...
  async initializeConfirmationNumber() {
    ...
    if (confNumberField) {
      ...
      while (retryCount < maxRetries && !nextConfNumber) {
        ...
        // We'll read it from the insert response after saving
        console.log('ℹ️ Confirmation number will be generated by the database');
        nextConfNumber = null; // Let the server assign it
  ...
  async saveReservation(opts = {}) {
    ...
    try {
      ...
      // Get current confirmation number - let server assign if this is a new reservation
      let currentConfNumber = document.getElementById("confNumber")?.value;
      if (!currentConfNumber || currentConfNumber === 'NEW') {
        console.log('ℹ️  New reservation - confirmation number will be generated by database');
        currentConfNumber = null; // Let the server assign it
      }
      ...
      try {
        ...
        // Omit confirmation_number if not set; let DB generate it
        const savePayload = {
          ...reservationData,
          accountId: accountId,
        ...
        // Only include confirmation_number if explicitly provided
        if (currentConfNumber) {
          savePayload.confirmationNumber = currentConfNumber;
          savePayload.confirmation_number = currentConfNumber;
        }
        
        const supabaseResult = await db.saveReservation(savePayload);
        ...
        if (supabaseResult && supabaseResult.success === false) {
          ...
          const savedConfNumber = supabaseResult.confirmation_number || (Array.isArray(supabaseResult) && supabaseResult[0]?.confirmation_number);
          if (savedConfNumber) {
            currentConfNumber = savedConfNumber;
            const confField = document.getElementById('confNumber');
            if (confField) confField.value = savedConfNumber;
            console.log('🔢 Confirmation number from server:', savedConfNumber);
          }
